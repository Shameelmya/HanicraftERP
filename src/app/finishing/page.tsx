...
export default function FinishingDashboard() {
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      setTasks(res.filter(t => t.type === "FINISHING"));
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
    ...
    <AppLayout>
      <div className="page-content">
        <div className="page-header">
          <h1 className="text-page-title">My Finishing Tasks</h1>
          <p className="text-muted mt-1">Manage your assigned finishing operations</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {tasks.map((task) => {
              ...
              return (
                <div key={task.id} className="card border-purple-200">
                  <div className="card-header flex justify-between items-start bg-purple-50/30">
                    <div>
                      <span className="badge badge-purple mb-2">{task.taskCode}</span>
                      <h3 className="font-semibold text-purple-900">{op?.stage ?? task.type}</h3>
                    </div>
